...
import { X, ArrowDownLeft, DollarSign, Wallet } from 'lucide-react';
...
export const PayReceivableModal: React.FC<PayReceivableModalProps> = ({
  ...
  const { payReceivable, accounts } = useApp();

  const outstanding = receivable ? Math.max(0, receivable.amount - receivable.paidAmount) : 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    payReceivable(receivable.id, amount, targetAccountId);
    onClose();
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">
                Catat Penerimaan Pembayaran Piutang
              </h3>
              <p className="text-[11px] text-slate-500">{receivable.invoiceNumber} • {receivable.customerName}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3 text-xs">
          <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl space-y-1">
            <div className="flex justify-between text-slate-500">
              ...
              <span>{formatRupiah(receivable.amount)}</span>
            </div>
            <div className="flex justify-between text-slate-500">
              <span>Sudah Dibayar:</span>
              <span>{formatRupiah(receivable.paidAmount)}</span>
            </div>
            <div className="flex justify-between font-bold text-slate-900 dark:text-white pt-1 border-t border-slate-200 dark:border-slate-700">
              ...
              <span className="text-emerald-600 dark:text-emerald-400">{formatRupiah(outstanding)}</span>
            </div>
          </div>

          <div>
            <label className="text-slate-600 dark:text-slate-300 font-medium mb-1 block">Nominal Diterima (Rp)</label>
            <input
              type="text"
            ...
            />
            <div className="text-[11px] text-slate-400 mt-1">Kosongkan untuk melunasi seluruh sisa tagihan.</div>
          </div>

          <div>
            <label className="text-slate-600 dark:text-slate-300 font-medium mb-1 block">Masuk ke Kas / Rekening Bank</label>
            <select
